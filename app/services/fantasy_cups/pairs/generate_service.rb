...
module FantasyCups
  module Pairs
    class GenerateService
      ...
      def initialize(winner_detect_service: ::FantasyCups::Pairs::WinnerDetectService)
      ...
      def call(week:)
        ...
        week.season.all_fantasy_leagues.general.each do |fantasy_league|
          next unless fantasy_league.fantasy_cup

          @cups_round = fantasy_league.fantasy_cup.fantasy_cups_rounds.find_by(week: week)
          next unless @cups_round

          @fantasy_league = fantasy_league
          previous_cups_round =
            @fantasy_league.fantasy_cup.fantasy_cups_rounds.find_by(position: @cups_round.position + 1)
      ...
      def best_fantasy_teams
        @fantasy_league
          .fantasy_teams
          .order(points: :desc)
          .first(FantasyCups::CreateService::ROUND_VALUES[@cups_round.position][:teams_amount])
      end
      ...
      def create_pairs(lineup_ids)
        ...
        FantasyCups::Pair.upsert_all(pairs) if pairs.any?
      end
    end
  end
end
