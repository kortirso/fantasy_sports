...
describe Auth::Providers::Google do
  ...
  let(:code) { 'code' }
  let(:user_response) { { 'sub' => 1, 'email' => 'email' } }

  before do
    allow(FantasySports::Container.resolve('api.google.auth_client')).to(
      receive(:fetch_access_token).and_return(token_response)
    )
    allow(FantasySports::Container.resolve('api.google.client')).to(
